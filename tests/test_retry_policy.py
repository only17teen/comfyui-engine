...
async def test_exhausted() -> None:
    async def fail() -> None: raise RuntimeError("always")
...
async def test_decorator() -> None:
    ...
    async def fn() -> int:
        nonlocal count; count += 1
        if count < 2: raise RuntimeError("once")
        return 99
...
async def test_deadline_aborts() -> None:
    count = 0
    async def fail() -> None: nonlocal count; count += 1; raise RuntimeError("fail")
    async with DeadlineContext(0.05):
        with pytest.raises(RetryExhaustedError): await RetryPolicy(10, base_delay=1.0, jitter=False).execute(fail)
    assert count < 10
