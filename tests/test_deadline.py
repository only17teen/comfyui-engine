...
@pytest.mark.asyncio
async def test_none_outside() -> None:
    assert get_deadline() is None and remaining_time() is None


@pytest.mark.asyncio
async def test_sets_var() -> None:
    async with DeadlineContext(5.0):
...
async def test_narrowing() -> None:
    async with DeadlineContext(10.0) as outer:
        ...
        async with DeadlineContext(30.0):
            assert get_deadline() == outer_exp
        async with DeadlineContext(1.0):
            assert get_deadline() < outer_exp
