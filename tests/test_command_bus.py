...
class EchoHandler(CommandHandler[EchoCmd, str]):
    async def handle(self, command: EchoCmd) -> str:
        return f"echo:{command.message}"


class ValidatedHandler(CommandHandler[ValidatedCmd, int]):
    async def handle(self, command: ValidatedCmd) -> int:
...
async def test_validation_passes() -> None:
    ...
    bus.register(ValidatedCmd, ValidatedHandler())
...
async def test_validation_rejects() -> None:
    ...
    with pytest.raises(ValueError):
        await bus.dispatch(ValidatedCmd(value=-1))
