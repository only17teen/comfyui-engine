...
__all__ = ["Command","CommandHandler","CommandBus","logging_middleware","timing_middleware","validation_middleware"]
...
class CommandBus:
    ...
    def register(self, t: type, h: CommandHandler[Any, Any]) -> None: self._handlers[t] = h
    def use(self, mw: Middleware) -> None: self._middleware.append(mw)
    async def dispatch(self, command: Any) -> Any:
        handler = self._handlers.get(type(command))
        if handler is None:
            raise KeyError(f"No handler for {type(command).__name__}")
        async def base(cmd: Any) -> Any: return await handler.handle(cmd)
...
async def logging_middleware(command: Any, nxt: Dispatch) -> Any:
    try:
        ...
        r = await nxt(command); log.info("command.succeeded"); return r
    except ImportError:
        return await nxt(command)

async def timing_middleware(command: Any, nxt: Dispatch) -> Any:
    t = time.perf_counter()
    try: return await nxt(command)
    finally:
        try:
            from opentelemetry import metrics as m
            m.get_meter("comfyui_engine").create_histogram("command.duration_ms",unit="ms").record((time.perf_counter()-t)*1000,{"command":type(command).__name__})
