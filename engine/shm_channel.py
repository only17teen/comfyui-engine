...
class ShmHeader(ctypes.Structure):
    _pack_ = 8
    _fields_ = [("write_seq",ctypes.c_uint64),("read_seq",ctypes.c_uint64),("data_len",ctypes.c_uint64),("_pad",ctypes.c_uint8*40)]
...
class ShmChannel:
    def __init__(self, name: str, size: int=1024*1024, *, create: bool=False) -> None:
        self._name = name; self._data_size = size
    ...
    def write(self, data: bytes|bytearray|memoryview) -> None:
        n = len(data)
        if n > self._data_size: raise ValueError(f"payload too large: {n} > {self._data_size}")
        self._data_view[:n] = data
        self._header.data_len = n; self._header.write_seq = self._header.write_seq + 1
    def read_if_new(self) -> bytes|None:
        ws, rs, n = self._header.write_seq, self._header.read_seq, self._header.data_len
        if ws == rs: return None
        payload = bytes(self._data_view[:n]); self._header.read_seq = ws; return payload
    ...
    def close(self) -> None:
        try: self._data_view.release()
        except Exception: pass
        self._shm.close()
    def unlink(self) -> None:
        try: self._shm.unlink()
        except Exception: pass
    def __enter__(self) -> "ShmChannel": return self
    def __exit__(self, *_: object) -> None: self.close(); self.unlink() if self._owned else None
    @property
    def name(self) -> str: return self._name
    @property
    def data_size(self) -> int: return self._data_size
