...
class QueryBus:
    ...
    def _get_lock(self) -> asyncio.Lock:
        ...
        return self._lock
    def register(self, t: type, h: QueryHandler[Any, Any]) -> None: self._handlers[t] = h
    async def ask(self, query: Any) -> Any:
        handler = self._handlers.get(type(query))
        if handler is None: raise KeyError(f"No handler for {type(query).__name__}")
    ...
    async def ask_cached(self, query: Any, cache_key: str) -> Any:
        ...
        async with self._get_lock():
            ...
            self._cache[cache_key] = result; return result
    def invalidate(self, key: str) -> None: self._cache.pop(key, None)
