...
class TracingDashboard:
    ...
    def _parse_trace_summary(self, trace_data: Dict[str, Any]) -> TraceSummary:
        ...
        services = list(set(span.get("process", {}).get("serviceName", "unknown") for span in spans))
        ...
        warnings = sum(
            1 for span in spans
            if any(tag.get("key") == "warning" and tag.get("value") for tag in span.get("tags", []))
        )
...
_global_tracing_dashboard: Optional[TracingDashboard] = None


def get_tracing_dashboard() -> Optional[TracingDashboard]:
