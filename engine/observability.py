...
try:
    ...
    _STRUCTLOG = False
try:
    from engine.metrics import get_registry as _get_registry
...
class _N:
    ...
    def get_span_context(self) -> _NC:
        return _NC()

    def __enter__(self) -> _N:
        return self
...
class Span:
    ...
    def set_attribute(self, key: str, value: Any) -> None:
        try:
            ...
            pass
        try:
            self._log = self._log.bind(**{key: value})
        except Exception:
            pass
    ...
    def record_metric(
        ...
        try:
            ...
            if value == int(value) and value >= 0:
                r.request_counter().add(int(value), attrs or {})
            else:
                r.request_duration().record(value, attrs or {})
        except Exception:
            pass
    ...
    def set_error(self, exc: Exception) -> None:
        ...
        try:
            ...
            self._otel_span.set_status(StatusCode.ERROR, str(exc))
            self._otel_span.record_exception(exc)
        except Exception:
            pass
        try:
            self._log.error(f"{self.name}.error", error=str(exc))
        except Exception:
            pass

    @property
    def elapsed_ms(self) -> float:
        return (time.perf_counter() - self._start_ns) * 1000
...
async def observe(
    ...
    if _OTEL:
        try:
            tracer = _otel_trace.get_tracer("comfyui_engine")
            otel_ctx = tracer.start_as_current_span(name)
            otel_span = otel_ctx.__enter__()
            if attributes:
                for k, v in attributes.items():
                    otel_span.set_attribute(k, v)
            sc = otel_span.get_span_context()
            trace_id = format(sc.trace_id, "032x") if sc.is_valid else "noop"
        except Exception:
            otel_span = _N()
            otel_ctx = None
    log: Any = _NL()
    ...
    try:
        ...
        try:
            ...
            pass
        try:
            log.info(f"{name}.done", duration_ms=round(span.elapsed_ms, 2))
        except Exception:
            pass
