...
class CloudProviderClient(ABC):
    ...
    @abstractmethod
    async def get_instance_status(self, instance_id: str) -> Optional[ProvisionedInstance]:
        """Get instance status."""
        pass
...
class AWSClient(CloudProviderClient):
    ...
    async def _wait_for_spot_instance(self, spot_request_id: str, timeout: int = 300) -> str:
        ...
        while time.time() - start_time < timeout:
            ...
            elif status in ["capacity-not-available", "capacity-oversubscribed", "price-too-low"]:
...
class GCPClient(CloudProviderClient):
    ...
    async def provision_instance(
        ...
        try:
            ...
            labels.update({k.lower().replace("-", "_")[:63]: v.lower().replace("-", "_")[:63]
                          for k, v in (tags or {}).items()})
            ...
            if ssh_key:
                metadata_items.append(compute_v1.Items(
                    key="ssh-keys",
    ...
    async def get_instance_status(self, instance_id: str) -> Optional[ProvisionedInstance]:
        ...
        try:
            ...
            return ProvisionedInstance(
                ...
                ssh_user="ubuntu",
                launch_time=instance.creation_timestamp.timestamp() if hasattr(instance.creation_timestamp, 'timestamp') else time.time(),
