...
class ModelInfo:
    ...
    _model_ref: Optional[weakref.ref] = None
...
class ModelCache:
    ...
    async def _ensure_space(self, needed_mb: float) -> None:
        ...
        while (self._current_memory_mb + needed_mb > self.max_memory_mb or
               len(self._cache) >= self.max_models) and self._cache:
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        # Example warmup
        await cache.warmup([
            {"name": "sdxl_base", "path": "/models/sdxl.safetensors", "type": "checkpoint"},
            {"name": "anime_lora", "path": "/models/anime.safetensors", "type": "lora"},
        ])
